<script lang="ts">
    ...
    request("stats").then((result) => {
        categories = result.categories.sort((a:string,b:string)=>a.toLocaleLowerCase().localeCompare(b.toLocaleLowerCase()));
    });
    let current: post = {
        heading: "",
        teaser: "",
    ...
    function newCat(){
        ...
        if (newcat) {
            categories=[...categories, newcat].sort((a,b)=>a.toLocaleLowerCase().localeCompare(b.toLocaleLowerCase()));
...
<div class="flex flex-col prose m-2">
    <div class="flex flex-row w-full ml-2 pr-5">
        ...
        <button class="btn ml-2" on:click={newCat}
            >{$_("newcat")}</button>
    </div>
    <input
        class="field"
